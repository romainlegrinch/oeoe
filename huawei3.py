...
class Slice:
    ...
    packet_count: int = 0  # Added to track number of processed packets
...
class NetworkSliceScheduler:
    ...
    def can_schedule_packet(self, packet: Packet, slice_obj: Slice, start_time: int) -> bool:
        ...
        # Vérification souple du délai maximal
        if end_time - packet.arrival_time > slice_obj.max_delay * 1.1:  # 10% de marge
            return False
            
        # Vérification souple de la bande passante
        ...
        if duration > 0:
            ...
            if achieved_bandwidth < target_bandwidth * 0.9:  # 10% de marge
                return False
    ...
    def schedule_packets(self):
        ready_packets = []
        delayed_packets = []  # Pour les paquets temporairement non planifiables
        ...
        while True:
            ...
            for slice_obj in self.slices:
                while slice_obj.has_more_packets():
                    ...
                    if self.can_schedule_packet(current_packet, slice_obj, start_time):
                        ...
                        heapq.heappush(ready_packets, (-priority, current_packet.arrival_time, slice_obj, current_packet))
                    else:
                        delayed_packets.append((slice_obj, current_packet))
                    slice_obj.get_next_packet()
            
            # Essayer de planifier les paquets retardés
            remaining_delayed = []
            for slice_obj, packet in delayed_packets:
                ...
                if self.can_schedule_packet(packet, slice_obj, start_time):
                    ...
                    heapq.heappush(ready_packets, (-priority, packet.arrival_time, slice_obj, packet))
                else:
            ...
            # Planifier le paquet de plus haute priorité
            _, _, slice_obj, packet = heapq.heappop(ready_packets)
            transmission_time = self.calculate_transmission_time(packet.size)
            ...
            slice_obj.last_departure_time = end_time
